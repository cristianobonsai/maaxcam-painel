...
export default function CameraCartao() {
  ...
  // Carga completa: roda no início e depois de salvar itens (sincroniza tudo com o servidor).
  ...
  // Carga leve: só atualiza status (enabled/paths), sem mexer nos itens/duração
  // que o usuário pode estar editando na tela (evita perder edição em andamento).
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-6 space-y-6 relative z-10">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="font-display text-2xl font-bold text-white">Cartão — {data.name || data.camera_id}</h1>
            <div className="text-xs text-slate-500">{data.camera_id}</div>
          </div>
          {tab === 'intro' && (
            <span className={`rounded-full px-3 py-1 text-xs font-medium ${data.card_enabled ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-600/40 text-slate-300'}`}>
